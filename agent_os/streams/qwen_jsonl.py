...
- `quota_verdict`: the stream carries no `rate_limit_event`, so the verdict reads `allowed` unless
  the terminal `result` itself says `is_error` with an `api_error_status` -- the same detector a
  Claude run is read with, kept unchanged here (#514 stage 1 changes no behaviour). That the guard
  CUTS only a Claude run on it is the guard's rule, not this parser's.
