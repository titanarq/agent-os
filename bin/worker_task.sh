...
PROMPT
# The one human's login is project config, never a literal in a script
# (docs/adr/2026-09-14-the-agent-mechanism-is-project-agnostic-and-configured-not-coded.md); the
# RULES heredoc is quoted, so it is substituted here rather than expanded there. The main
# checkout's path is derived, not configured -- see main_checkout() above.
RULES=${RULES//__HUMAN_LOGIN__/$(project_value human_login)}
RULES=${RULES//__HUMAN_MESSAGE_RULES__/$("$agent_python" -m agent_os.lib human-message-rules)}
RULES=${RULES//__MAIN_CHECKOUT__/$(main_checkout)}
RULES=${RULES//__TEST_COMMAND__/$(project_value test_command)}

# RULES paragraphs of their own, rather than a word inside one, and all rendered from
# config/agents.yaml: the two lists a worker's diff is audited against, the commands this project
# forbids and the environment it exports into the worker's own process. Any can come back empty,
# and `agent_substitute_rules_paragraph` (agent_os/bin/agent_task.sh, sourced above) then removes the
# placeholder's own line together with the blank one after it, so a project that protects no path
# keeps the single blank line every other section boundary has instead of a stub heading -- or a
# gap twice as wide as the one it fills. The environment paragraph reads the same
# `project.worker_environment` the export loop below reads, so the sentence that used to spell the
# shared server's port here is gone rather than maintained beside it (`docs/AGENT_OS.md` §7 row (a)).
#
# The two path paragraphs are the one exception to "each stands on its own": together they state
# the ownership rule as a contrast -- these no brief can authorize, those a brief can -- and each
# is what makes the other worth reading, so the pair renders whole or not at all (#390,
# docs/adr/2026-09-16-the-mechanisms-own-files-are-not-the-host-projects-protected-paths.md). The
# gate reads the two lists through the `FORBIDDEN` and `MECHANISM` patterns built above, each of
# which is empty exactly when its own list is, so a config missing either one tells the worker
# nothing about file ownership while `collect` below still reads the two lists on its own and
# audits whichever half has a pattern: this narrows what a worker reads, never what it is judged by.
if [ -n "$FORBIDDEN" ] && [ -n "$MECHANISM" ]; then
  forbidden_rules=$("$agent_python" -m agent_os.lib forbidden-paths-rules)
  mechanism_rules=$("$agent_python" -m agent_os.lib mechanism-paths-rules)
else
  forbidden_rules=""
  mechanism_rules=""
fi
agent_substitute_rules_paragraph __FORBIDDEN_PATHS_RULES__ "$forbidden_rules"
agent_substitute_rules_paragraph __MECHANISM_PATHS_RULES__ "$mechanism_rules"
agent_substitute_rules_paragraph __NEVER_RUN_RULES__ \
  "$("$agent_python" -m agent_os.lib never-run-rules)"
agent_substitute_rules_paragraph __WORKER_ENVIRONMENT_RULES__ \
  "$("$agent_python" -m agent_os.lib worker-environment-rules)"
